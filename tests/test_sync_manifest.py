...
def test_run_sync_minimal(tmp_path: Path, monkeypatch):
    ...
    with mock.patch.object(
        sm,
        "detect_game_patch",
        return_value=sm.StepResult("game_patch", "ok", "", {"patched": False}),
    ):
        with mock.patch.object(
            sm,
            "extract_ledger",
            return_value=sm.StepResult("ledger_extract", "ok", "", {"stats": fake_stats}),
        ):
            with mock.patch.object(
                sm,
                "build_relations",
                return_value=sm.StepResult("relations", "skipped", "no items"),
            ):
                with mock.patch.object(
                    sm, "build_site_bundles", return_value=sm.StepResult("site_bundles", "ok", "")
                ):
                    with mock.patch.object(
                        sm, "package_upload", return_value=sm.StepResult("upload", "skipped", "")
                    ):
                        report = sm.run_sync(
                            locallow=locallow, data_dir=data, site_dir=site, upload=False
                        )
