...
def build_drops(
    items: list[dict],
    mobs: list[dict],
    ledger_drops: list[dict] | None = None,
    crowd_drops: list[dict] | None = None,
) -> list[dict]:
    ...
    for row in crowd_drops or []:
        item_t = (
            resolve_item(row.get("item_name", "") or row.get("item_title", ""), item_canon)
            or row.get("item_name")
            or row.get("item_title")
        )
        mob_t = (
            resolve_mob(row.get("mob_name", "") or row.get("mob_title", ""), mob_canon)
            or row.get("mob_name")
            or row.get("mob_title")
        )
        zone = normalize_zone_name(row.get("zone"))
        add(
            item_t,
    ...
    for d in drops:
        conn.execute(
            ...
            (
                ...
                int(d["via_mob"]),
                int(d["via_item"]),
                int(d.get("via_client", False)),
                int(d.get("via_ledger", False)),
                int(d.get("via_crowd", False)),
                int(d.get("observations", 0)),
                int(d.get("contributors", 0)),
                float(d.get("confidence", 0.0)),
                d.get("status"),
                int(d.get("conflict", False)),
            ),
        )
...
def main() -> int:
    ...
    (DATA / "drops.json").write_text(
        json.dumps(drops, indent=2, ensure_ascii=False), encoding="utf-8"
    )
    (DATA / "zones.json").write_text(
    ...
    ledger_only = sum(
        1 for d in drops if d.get("via_ledger") and not d["via_mob"] and not d["via_item"]
    )
    ledger_confirmed = sum(
        1 for d in drops if d.get("via_ledger") and (d["via_mob"] or d["via_item"])
    )
    ...
    print(
        f"Relations: {len(drops)} drop links ({linked_items} items, {linked_mobs} mobs, {both} confirmed both sides)"
    )
    if ledger_drops:
        print(
            f"  Ledger: {len(ledger_drops)} raw links -> {ledger_confirmed} confirmed wiki, {ledger_only} ledger-only"
        )
    if crowd_drops:
        print(f"  Crowd: {len(crowd_drops)} aggregated links merged")
    print(
        f"  Provenance: {confirmed} confirmed, {via_client} client-encoded, "
        f"{crowd_cand} crowd-candidates (wiki gaps), {conflicts} conflicts to review; "
        f"avg confidence {avg_conf:.2f}"
    )
    print(
        f"Zones indexed: {len(zones)} (canonical: {audit['canonical']}, with mobs: {audit['zones_with_mobs']})"
    )
